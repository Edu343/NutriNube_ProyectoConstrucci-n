...
public class ConsultaFormularioViewLayout extends JPanel {
    ...
    private JTextField txtEdadField;
    private JComboBox<String> cmbSexo; // CAMBIO: Ahora es JComboBox
    ...
    private void initComponents() {
        ...
        personalInfoPanel = createPersonalInfoPanel("Información Personal",
                new String[] { "Nombre(s)", "Apellido", "Edad", "Sexo", "Correo Electrónico", "Altura", "Peso" });
    ...
    private JPanel createSectionPanel(String title, String[] fieldNames) {
        ...
        for (int i = 0; i < fieldNames.length; i++) {
            ...
            // Lógica para decidir si crear JTextField o JComboBox
            if (title.equals("Información Personal") && fieldNames[i].equals("Sexo")) {
                inputComponent = createComboBox(new String[] { "Hombre", "Mujer" });
                cmbSexo = (JComboBox<String>) inputComponent; // Asignación al nuevo JComboBox
            } else {
                inputComponent = createTextField();
                
                // Asignación de campos de texto a variables de clase (ajustada)
                if (title.equals("Información Personal")) {
                    if (fieldNames[i].equals("Nombre(s)")) txtNombreField = (JTextField) inputComponent;
                    if (fieldNames[i].equals("Apellido")) txtApellidoField = (JTextField) inputComponent;
                    if (fieldNames[i].equals("Edad")) txtEdadField = (JTextField) inputComponent;
                    // Sexo se maneja arriba
                    if (fieldNames[i].equals("Correo Electrónico")) txtCorreoField = (JTextField) inputComponent;
                    if (fieldNames[i].equals("Peso")) pesoField = (JTextField) inputComponent;
                    if (fieldNames[i].equals("Altura")) alturaField = (JTextField) inputComponent;
                } else if (title.equals("Historial Médico")) {
                    if (fieldNames[i].equals("Condiciones Médicas")) txtCondicionesMedicasField = (JTextField) inputComponent;
                    if (fieldNames[i].equals("Medicación")) txtMedicacionField = (JTextField) inputComponent;
                    if (fieldNames[i].equals("Historial de cirugías")) txtHistorialCirugias = (JTextField) inputComponent;
                } else if (title.equals("Historial Alimenticio")) {
                    if (fieldNames[i].equals("Alergias")) txtAlergiasField = (JTextField) inputComponent;
                } else if (title.equals("Estilo de Vida")) {
                    if (fieldNames[i].equals("Horario de sueño")) txtHorarioSueno = (JTextField) inputComponent;
                    if (fieldNames[i].equals("Hábitos alimenticios")) txtHabitoAlimenticio = (JTextField) inputComponent;
                } else if (title.equals("Consumo de Líquidos")) {
                    if (fieldNames[i].equals("Tipos de líquidos consumidos")) txtTipoLiquidoConsumido = (JTextField) inputComponent;
                    if (fieldNames[i].equals("Cantidad de líquidos consumidos")) txtCantidadLiquido = (JTextField) inputComponent;
                } else if (title.equals("Disposición")) {
                    if (fieldNames[i].equals("Alguna barrera para seguir el plan")) txtBarreraAlimenticia = (JTextField) inputComponent;
                }
            }
    ...
    public void clearAllFields() {
        // La lista de JTextFields se ha ajustado, cmbSexo ya no es un JTextField
        JTextField[] textFields = {
            txtNombreField, txtApellidoField, txtEdadField, txtCorreoField,
        ...
        // Se añade cmbSexo a la lista de JComboBoxes para limpieza
        JComboBox<?>[] comboBoxes = {
            cmbSexo, cmbNivelActividad, cmbRazonConsulta, cmbPreferenciaComida, cmbNivelEstres
        };
    ...
    public JTextField getTxtEdadField() {
        return txtEdadField;
    }

    // CAMBIO: Nuevo Getter para JComboBox Sexo
    public JComboBox<String> getCmbSexo() {
        return cmbSexo;
    }
    
    // Eliminado el viejo getTxtSexoField()
